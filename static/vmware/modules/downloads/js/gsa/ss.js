...
function ss_suggest(qVal) {
  ...
  var url = ss_gsa_host ? location.protocol+'//' + ss_gsa_host + uri : uri;
  if (ss_panic) {
        ...
        try {
          switch (ss_protocol) {
            ...
            default:
			  ...
			  suggestions = ss_filter(suggestions);
...
//Filter the inmeta data range terms
function ss_filter(suggestions) {
	try	{
	var filtered = [];
	for(var i=0; i < suggestions.length; i++) {
		var term = suggestions[i];
		if(term.match(/inmeta:dsca_rdate:daterange:\d{4}-\d{2}-\d{2}\.\.\d{4}-\d{2}-\d{2}/)) {
			continue;
		}
		filtered.push(term);
	}
...
function ss_show(qry) {
  ...
  if (cnt > 0) {
    ...
    var closeText = (typeof(downloadsSearch.globalVar.close) != "undefined") ? downloadsSearch.globalVar.close : "close";
    clsTxt.appendChild(document.createTextNode(closeText));
