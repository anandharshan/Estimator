...
(function ($) {
    ...
    $.fn.contextMenu = {
        ...
        init: function (options) {
            ...
            this.buildContextMenu(config.data, config.cmenuId, config.cClass, config.showBtn);
            this.bindContextMenuEvents(config.data, config.cmenuId, config.getTarget, config.showBtn);
            ...
            if(config.actionBtnFlg){
                this.createActionBtn(config.targetElem, config.cmenuId, config.menuChgState, config.getTargetDetails, config.getTargetNode, config.funcName, config.showBtn);
            }
        },
        buildContextMenu: function (mapObj, cmenuId, cClass, showBtn) {
			if(!$('#' + cmenuId).length){
				...
				if(!showBtn) $('#' + cmenuId).find('ul li').filter(':first').addClass('paddingT10').end().filter(':last').addClass('paddingB10');
			}
        },
        bindContextMenuEvents: function (mapObj, cmenuId, getTarget, showBtn) {
            var cmenu = $('#' + cmenuId);
			...
			$(document).mousedown(function(e){
				if(cmenu.has(e.target).length){
					...
					if(!showBtn){
						...
						getTarget('',config.activeCls,config.activeBtnCls);
					}else{
						targetNode.closest('tr').removeClass('active');
						targetNode.removeClass('active').find('.'+config.activeBtnCls).removeClass(config.activeBtnCls).children().removeClass(config.activeBtnCls);
					}
				}
			});
        ...
        createActionBtn: function (targetElem, cmenuId, menuChangeState, getTargetDetails, getTargetNode, funcName, showBtn) {
                var cmenu = $('#' + cmenuId),
                ...
                if (contextMenuFlag) {
                    if (e.which === 3 && targetElement.has(e.target).length && !$(e.target).hasClass('disabled') && !$(e.target).attr('readonly')) {
                        $(e.target).unbind('contextmenu').bind("contextmenu", function (e) {
							...
							if(targetNode.children().filter('.'+config.activeCls).length){
								targetNode.children().removeClass(config.activeCls);
