...
.controller('configureAddonsCtrl',['$scope','$state','$modal','newServiceFactory', '$window','confirmDialog',
function($scope, $state, $modal, newServiceFactory, $window, confirmDialog){
	...
	$scope.displayAddOnData = function(getData){
		//$scope.$apply(function(){			// remove this for dev-15
			...
			angular.copy($scope.addOnData.sppTokenToMSRP,newServiceFactory.tokenToMSRPConversion);
	...
	$scope.goToViewEstimator = function(){
		if(typeof riaLinkmy !="undefined") riaLinkmy("trial-to-paid : build-estimate : additional-capacity : continue");
			...
			function(successData){
				$scope.$apply(function(){
					if(!$scope.isEmpty(successData.ERROR_MESSAGE)){
						...
						newServiceFactory.generatedCartId = successData.wrapper.trialSid;
		...
		// angular.copy($scope.addOnData, newServiceFactory.addOnObj);
		// angular.copy($scope.addOnData, newServiceFactory.viewEstimateObj);
		// //newServiceFactory.generatedCartId = successData.wrapper.trialID;
		// $state.go('estimatorSteps.viewEstimate');
