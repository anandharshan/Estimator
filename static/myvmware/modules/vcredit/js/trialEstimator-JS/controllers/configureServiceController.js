...
.controller('configureServiceCtrl',['$scope','$state','$modal', 'newServiceFactory', '$timeout', '$window', 'confirmDialog',
function($scope, $state, $modal, newServiceFactory, $timeout, $window, confirmDialog){
	...
	$scope.pageInit = function(){
		...
		if($.isEmptyObject(newServiceFactory.configureService) == true){
			...
			// var getServicesUrlMod = $scope.globalVars.step3ConfigureServiceURL;
	...
	$scope.displayServiceData = function(getData){
		...
		if(Object.keys($scope.serviceData.region[0]).length == 1){
			$scope.serviceData.region[1] = Object.keys($scope.serviceData.region[0])[0];
	...
	$scope.setCrosssku = function(){
		...
		if(!$scope.isEmpty($scope.serviceData.region[1])
			...
			&& !$scope.isEmpty($scope.serviceData.storageType[$scope.serviceData.storageType.length-1])){
				...
				// // Code check for editions
				// if($scope.serviceData.editions != undefined && $scope.serviceData.editions != null && $scope.serviceData.editions.length > 0){
				// 	if($scope.isEmpty($scope.serviceData.editions[1])){
				// 		$scope.skuError = true;
				// 	}
				// }
	...
	// $scope.changeEditions = function(editions){
	...
	$scope.goToAddOns = function(){
		if(typeof riaLinkmy !="undefined") riaLinkmy("trial-to-paid : build-estimate : configure-Service : continue");

		angular.copy($scope.serviceData, newServiceFactory.configureService);
		
		newServiceFactory.addOnObj = {};
		$scope.ajaxLoader = true;
		$scope.addonServiceData = $scope.SubmitServiceData + "&_VM_serviceInstanceId=" + $scope.globalVars.encodedTrialID;
		...
		newServiceFactory.postServices(
			...
			$scope.globalVars.step4AddadditionalCapacityURL,    		// This comes from the jsp config object
