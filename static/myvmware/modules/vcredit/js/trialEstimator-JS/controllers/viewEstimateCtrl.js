...
.controller('viewEstimateCtrl', ['$scope', '$state', '$modal', 'newServiceFactory','confirmDialog', '$window', function($scope, $state, $modal, newServiceFactory, confirmDialog, $window){
	...
	if($.isEmptyObject(newServiceFactory.configureService)){
		...
		//$scope.trialID = newServiceFactory.generatedCartId;
	}
	...
	$scope.updateTermType = function(){
		...
		newServiceFactory.postServices(
			...
			$scope.globalVars.step5ViewEstimateUpdateClickURL,    		// This comes from the jsp config object
			"_vm_skuDetailList=" + JSON.stringify($scope.postObj) + "&_VM_serviceInstanceId=" + $scope.globalVars.encodedTrialID              // changed scope variable to add secondary admin....
			,'GET'
		);
	...
	if($.isEmptyObject(newServiceFactory.viewEstimateObj)){
		/*newServiceFactory.postServices(
			...
			$scope.globalVars.step5ViewEstimateURL,    		// This comes from the jsp config object
	...
	$scope.disableEdit=function($event, addOnKey, addOns, index){
		...
		newServiceFactory.postServices(
			...
			"_vm_skuDetailList=" + JSON.stringify($scope.postObj) +"&_VM_serviceInstanceId=" + $scope.globalVars.encodedTrialID
			,'GET'
		);
		// $scope.ajaxLoader = false;
		// vmf.msg.page("");
...
//console.log($scope.validSelectedData);		
	...
	$scope.removeAddon = function($event, addOnKey, addOns){
		...
		.result.then(function(btn){
				...
				newServiceFactory.postServices(
					...
					"_vm_coreID=" + newServiceFactory.currentCartId + "&_vm_addonSku=" + addOns[8].selectedAddonSKU[0]+"&_VM_serviceInstanceId=" + $scope.globalVars.encodedTrialID
					,'GET'
				);
				// angular.element($event.target).siblings('.ajaxSpinner').addClass('hide');
		  // 		$scope.ajaxLoader = false;
	...
	$scope.submitOrder=function($event, goTo){
		...
		if(typeof riaLinkmy !="undefined") riaLinkmy("trial-to-paid : build-estimate : view-estimate : export-to-excel");
		...
		submitPostData = '_vm_skuDetailList='+JSON.stringify($scope.postObj)+"&_VM_serviceInstanceId=" + $scope.globalVars.encodedTrialID;

		if(goTo == 'excel'){
			$scope.excelImportDownloadURL = step3SubmitOrderUrl + "&_vm_coreID="+newServiceFactory.currentCartId+"&" + submitPostData;
		}else{
			$scope.ajaxLoader = true;
			vmf.msg.page("");
	...
	$scope.cancelFn = function(){
		if(typeof riaLinkmy !="undefined") riaLinkmy("trial-to-paid : build-estimate : view-estimate : close");
		newServiceFactory.clearObjects();
		window.location = $scope.globalVars.closeButtonURL;
	}
