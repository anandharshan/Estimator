...
ice.matrixaccess = { 
	...
	init : function(arg1, agr2,arg3, arg4, arg5){
		...
		$('#exportAllToCsvButton').click(function() {
			...
			ice.matrixaccess.generateCSVreports($exportToCsvActionFromMatrixViewUrl, _fPerPostData, "users-permissions : matrix-access : export-all", "users-permissions : matrix-access : Export-to-CSV : Error");
			
		});
		callBack.addsc({'f':'riaLinkmy','args':['users-permissions : spreadsheet-view']});
	},
		...
		vmf.ajax.post(url,postData,
			function(jData){//Success call
				...
				if(data.respUrl && data.respUrl != 'undefined' && data.respUrl.length > 0) {
					...
					riaLinkmy(failedOmnitureArgs);
					ice.matrixaccess.showError('exportCsvError',data.error);
				}
			},
			function(errMsg){
				riaLinkmy(failedOmnitureArgs);
			...
			for(var innerIndex = 0; innerIndex < listFolderVO.length; innerIndex++){
				...
				for(var permissionIndex = 0; permissionIndex < permissionVOList.length; permissionIndex++){
					if(permissionVOList[permissionIndex].assigned == true)	{
						outputHtml = outputHtml+'<td class="regular center"><img src="/static/myvmware/common/img/dot.png" title="'+$staticTextfortick+'" alt="'+ice.globalVars.yesLbl+'" /></td>';
					}else{
						outputHtml = outputHtml+'<td class="regular center"><img src="/static/myvmware/common/img/cross.png" title="'+$staticTextforcross+'" alt="'+ice.globalVars.noLbl+'" /></td>';
					}
				}
			}
			cnt++;
