...
(function($, window, document) {
	...
	$.fn.dataTable = function( oInit )
	{
		...
		function classSettings ()
		{
			...
			/*Custom Function created by Nagaraj
			* function : error
			* purpose : Exception Handling for all datatable requests
			*/
			this.error=null;
			this.errMsg=null;
			/*
			 ...
			 * Scope:    jQuery.dataTable.classSettings
			 */
			this.fnServerData = function ( url, data, callback, settings, type, errorCb) {
				settings.jqXHR = $.ajax( {
					"type":type || "GET",
					"url": url,
					"data": data,
		...
		this.fnAdjustColumnSizing = function ( bRedraw )
		{
			...
			if (oSettings._sWidth !=$(window).width()){
				setTimeout(function(){_storeWidthForResize(oSettings,true)},500);
				oSettings._sWidth =$(window).width();
			}
		};
		...
		function _fnDraw( oSettings )
		{
			...
			_storeWidthForResize(oSettings, false);
		}
		
		/* Function: _storeWidth 
		   Purpose: Resize - Store Width */
		
		function _storeWidthForResize(oSettings, flg){
			if(oSettings.sDom && oSettings.sDom.indexOf("z")!=-1){
				$.each(oSettings.aoColumns,function(i,v){
					if(v.bVisible && (($(v.nTh).data("origWidth")==undefined) || flg )) {
						$(v.nTh).data("origWidth",$(v.nTh).width()).css("width",$(v.nTh).width()+"px");
					}
				});
			}
		}
		...
		function _fnFeatureHtmlProcessing ( oSettings )
		{
			...
			//$(oSettings.nTable).parents('div.dataTables_wrapper').addClass('processingDisplayHeight');
		...
		function _fnProcessingDisplay ( oSettings, bShow )
		{
			if ( oSettings.oFeatures.bProcessing )
			{
				...
				for ( var i=0, iLen=an.length ; i<iLen ; i++ )
				{
					...
					//$(oSettings.nTable).parents('div.dataTables_wrapper').removeClass('processingDisplayHeight');
				}
			}
		}
		...
		return this.each(function()
		{
			...
			if ( typeof oInit != 'undefined' && oInit !== null )
			{
				...
				_fnMap( oSettings, oInit, "error" );
				_fnMap( oSettings, oInit, "errMsg" );
...
/* This is for displaying currency*/
jQuery.fn.dataTableExt.afnSortData['dom-currency'] = function  ( oSettings, iColumn )
{
    var aData = [];
	$( 'td:eq('+iColumn+')', oSettings.oApi._fnGetTrNodes(oSettings) ).each( function () {
		if ($(this).find('span').length) aData.push($(this).find('span').html())
		else aData.push( this.innerHTML );
	} ); 
	return aData;
}
...
$.fn.dataTableExt.oApi.fnReloadAjax = function ( oSettings, sNewSource, fnCallback, method, data, errorCb, bStandingRedraw )
{
	...
	var that = this;
	if(oSettings.jqXHR!=null) oSettings.jqXHR.abort(); //Aborting all the requests before initiating new request - BUG-00056069 && BUG-00056236
	/* Clear the old information from the table */
	that.oApi._fnClearTable( oSettings );
	that.find('tbody tr').hide();
	that.oApi._fnProcessingDisplay( oSettings, true );
	var iStart = oSettings._iDisplayStart;
	var data = data || [];
	oSettings.fnServerData( oSettings.sAjaxSource, data, function(json) {
		if(json.aaData){
			/* Got the data - add it to the table */
		...
		{
			fnCallback(that, oSettings, json);
		}
    }, oSettings, method, errorCb);
};
...
(function($, window, document) {
  ...
  ColResize.prototype = {
    ...
    "_fnMouseDown": function ( e, nTh )
    {
      ...
      nTh = $(nTh),
      pos = 0;
      if(oTable.find("tbody tr").length==1 && oTable.find("tbody tr td").hasClass("dataTables_empty")) return;
      ...
      that.cursorPos = nTh[0].offsetLeft+$(nTh).outerWidth(true);
  
      var nThNext = nTh.next();
      this.s.mouse.nextStartWidth = nThNext.width();
      that.dom.resize = true;

      that.divider = $("<div class='col-divider'></div>").appendTo(oTable.closest("div.dataTables_wrapper"));
      that.divider.css({"left":that.cursorPos});
      ...
      $(document).bind( 'mouseup.ColResize', function (e) {
        ...
        pos = that.cursorPos - parseInt(that.divider.css("left"),10);
        $(nTh).width(that.s.mouse.startWidth - pos +"px");
        $(nTh).next().width(that.s.mouse.nextStartWidth + pos + "px");
    ...
    "_fnSetColumnWidth" : function(nTh){
      ...
      $.each(aoColumns,function(i,v){
            if(v.bVisible) aoColumns[i].sWidth=$(v.nTh).width()+'px';
        });

      /* set it in datatable settings and redraw table */
      /*aoColumns[iColumnIndex].sWidth = nTh.width() +  'px';
      if(aoColumns[iColumnIndex+1]) aoColumns[iColumnIndex+1].sWidth = nTh.next().width() +  'px';
      if(aoColumns[iColumnIndex-1]) aoColumns[iColumnIndex-1].sWidth = nTh.prev().width() +  'px';*/
      $(oTable).dataTable().fnDraw(oSettings);
    },
    ...
    "_fnMouseMove": function ( e )
    {
      ...
      if (this.dom.resize) {
        ...
        if (moveLength != 0){
            if(moveLength<0 && ($(nTh).width() + moveLength >= $(nTh).data("origWidth")*.7))
                that.divider.css({"left":that.cursorPos + moveLength})
            else if (moveLength>0 && (nThNext.width() - moveLength >= nThNext.data("origWidth")*.7))
                that.divider.css({"left":that.cursorPos + moveLength})
        }
      }
    },
...
vmf.datatable = function($){
    ...
    return {
			...
			build: function(tableObj, config){
				/*if($(tableObj).dataTableSettings.length >0){$(tableObj).dataTable().fnDestroy()};*/
				this.tabObj = $(tableObj).dataTable(config); 
				//$(window).bind('resize', function () {$(tableObj).fnAdjustColumnSizing();});
			},
